    ...
    def load_config(self):
        ...
        try:
            ...
            for button_idx in range(12):  # Només els primers 12 botons per a modes
                ...
                # Assignar el mode al botó si existeix (MODE_CLASSES o registre JSON)
                mode_available = (mode_name in MODE_CLASSES or
                                  _get_mode_info_from_registry(mode_name) is not None)
                if mode_name and mode_available and mode_name not in disabled_modes:
                    self.button_mappings[button_idx] = mode_name
                    print(f"Botó {button_idx+1} assignat al mode '{mode_name}'")
                else:
                    # Si no està configurat o no existeix, no assignar res
                    if mode_name:
                        print(f"Botó {button_idx+1}: mode '{mode_name}' no trobat al registre")
